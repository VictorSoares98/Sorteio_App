...
// Alternar exibição de resultados do sorteio
const toggleCompletedStatus = () => {
  editedData.value.isCompleted = !editedData.value.isCompleted;
  
  // Se acabou de marcar como não concluído, limpar dados do vencedor
  if (!editedData.value.isCompleted) {
    editedData.value.winningNumber = null;
    editedData.value.winner = null;
    editedData.value.seller = null;
  } else if (!editedData.value.winner) {
    // Se acabou de marcar como concluído, inicializar dados vazios
    editedData.value.winningNumber = '';
    editedData.value.winner = {
      name: '',
      phone: '',
      congregation: ''
    };
    editedData.value.seller = {
      id: '',
      name: '',
      photoURL: ''
    };
    ...
    <form @submit.prevent="saveChanges" class="space-y-6">
      ...
      <div class="border-t border-gray-200 pt-6">
        <div class="flex items-center">
          <input 
            id="completed"
            v-model="editedData.isCompleted"
            type="checkbox"
            class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded"
            @change="toggleCompletedStatus"
          />
      ...
      <!-- Resultados do sorteio (apenas quando marcado como concluído) -->
      <div v-if="editedData.isCompleted" class="border-t border-gray-200 pt-6">
        <h3 class="text-lg font-medium text-primary mb-4">Resultados do Sorteio</h3>
        
        <div class="space-y-6">
          <!-- Número sorteado -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1" for="winningNumber">
              Número Sorteado
            </label>
            <input 
              id="winningNumber"
              v-model="editedData.winningNumber"
              type="text"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            <h4 class="font-medium text-gray-800 mb-3">Informações do Vencedor</h4>
            
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="winnerName">
                  Nome do Vencedor
                </label>
                <input 
                  id="winnerName"
                  v-model="editedData.winner.name"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                />
              </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="winnerPhone">
                  Telefone do Vencedor
                </label>
                <input 
                  id="winnerPhone"
                  v-model="editedData.winner.phone"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="winnerCongregation">
                  Congregação do Vencedor
                </label>
                <input 
                  id="winnerCongregation"
                  v-model="editedData.winner.congregation"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                />
              </div>
          ...
          <!-- Dados do vendedor -->
          <div class="bg-gray-50 p-4 rounded-lg">
            <h4 class="font-medium text-gray-800 mb-3">Informações do Vendedor</h4>
            
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="sellerName">
                  Nome do Vendedor
                </label>
                <input 
                  id="sellerName"
                  v-model="editedData.seller.name"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                />
              </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="sellerPhoto">
                  URL da Foto do Vendedor
                </label>
                <input 
                  id="sellerPhoto"
                  v-model="editedData.seller.photoURL"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
